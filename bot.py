...
async def send_game_keyboard(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if WEB_APP_URL:
        ...
        await context.bot.send_message(
            chat_id=chat_id,
            text="🎮 JuugTAPS is ready!\n\nPress the button below to open the game.",
            reply_markup=keyboard,
        )
    else:
        await context.bot.send_message(
            chat_id=chat_id,
            text="WEB_APP_URL is not configured on the bot server.",
        )


async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    arg = parse_start_arg(update)

    # Payment deep link used by the game.
    if arg == "buy5m":
...
async def send_stars_invoice(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await context.bot.send_invoice(
        chat_id=update.effective_chat.id,
        title="5M POINTS",
        description="Instant +5,000,000 JuugTAPS points",
        payload=payload,
        currency="XTR",
...
async def successful_payment(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await message.reply_text(
        "✅ Payment received!\n"
        "Your +5,000,000 points will appear in the game automatically."
    )
